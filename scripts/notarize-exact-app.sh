...
approval="WebKitUI-MCP-$release_version-$release_build-$expected_sha"
...
final_archive="$output_dir/WebKitUI-MCP-$release_version-notarized.zip"

if [ -e "$request" ]; then
  test "$(plutil -extract InputSHA256 raw "$request")" = "$expected_sha"
  test "$(plutil -extract TeamIdentifier raw "$request")" = "$expected_team"
  test "$(plutil -extract Version raw "$request")" = "$release_version"
  test "$(plutil -extract Build raw "$request")" = "$release_build"
else
  ...
  plutil -insert Version -string "$release_version" "$request"
  plutil -insert Build -string "$release_build" "$request"
...
if [ ! -e "$submission" ]; then
  printf '%s\n' \
    "Uploading WebKitUI MCP $release_version ($release_build), Team $expected_team, SHA-256 $expected_sha to Apple Notary Service."
...
COPYFILE_DISABLE=1 ditto -c -k --norsrc --noextattr --keepParent \
  "$app" "$final_archive"
if zipinfo -1 "$final_archive" | grep -Eq '(^__MACOSX/|(^|/)\._)'; then
  printf '%s\n' "notarized archive contains forbidden AppleDouble metadata" >&2
  exit 1
fi
