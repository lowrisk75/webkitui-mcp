...
public final class WebKitMCPServer {
  ...
  private func actuationConfirmationMessage(
    ...
    case .fill(let value):
      action =
        dispatchMode == .nativeAppKit
        ? "AppKit text insertion with exact value \(jsonQuoted(value)); site input handlers may autosave or cause server effects"
  ...
  private func executeActuation(
    ...
    if let uploadReceipt = runtime.latestFileUploadReceipt(),
      uploadReceipt.monotonicNanoseconds >= result.action.actionMonotonicNanoseconds
    {
